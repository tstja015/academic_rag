...
## README.md (updated section only — replace the Query syntax table)

```markdown
## Query modes

### Standard RAG (default)
Retrieves the most relevant chunks via SPECTER + HyDE + cross-encoder reranking.
Best for broad questions across many papers.

### Full-document mode  (`paper:` prefix)
Sends the **complete PDF text** directly to Claude — no chunking, no retrieval.
Identical to uploading the file.  Use this when you need:
- Deep analysis of a specipaper
...
paper:ADH: summarize          ← matches first PDF wh "ADH" in the filename
```
...
| `paper:<n1>,<n2>: ` | Full-doc | Send multiple complete PDFs |
| `methods:` | RAG | Restrict to methods sections |
| `results:` | RAG | Restrict to results sections |
| `folder:<name>:` | RAG | Restrict to one topic folder |
| `web:` | RAG | Add live web search |
| `summarize:<filename>` | RAG | Chunk-based summary |
